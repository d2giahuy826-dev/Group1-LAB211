...
public class RunSalaryCalculator {
    ...
    void testFulltime_DuNgay_CoOT() {
        ...
        double expectedBonus = Math.round(12_000_000.0 * 0.05);
        ...
        if (calc.getOvertimePay() > 0
                && Math.abs(calc.getAttendanceBonus() - expectedBonus) < 1
                && calc.getNetSalary() > 12_000_000) {
    ...
    void testNghiKhongPhep_MatBonus() {
        ...
        if (calc.getAttendanceBonus() == 0
                && calc.getAbsenceDeduction() > 0
                && calc.getNetSalary() > 12_000_000) {
            System.out.println("  PASS");
        } else {
            System.out.println("  FAIL");
